...
rasters_path <- "D:\\Dissertation\\Test_Area\\Attributes_RF2_test"
reference_raster <- "D:\\Dissertation\\Test_Area\\Attributes_RF2_test\\AWEinsh.tif"  # for resampling 
...
###########################################
# DIFFERENCE (IOU and MAPS)
###########################################

map2 <- raster("D:\\Dissertation\\Test_Area\\Saved\\RF2\\Class_cropped\\class_8.tif")
map1 <- raster("D:\\Dissertation\\Test_Area\\Saved\\RF1\\Classifications\\Class_2.tif")

# Define the intersection extent
extent_intersec <- intersect(extent(map1), extent(map2))
map1 <- crop(map1, extent_intersec)
map2 <- crop(map2, extent_intersec)

# Resample map2 to match map1 if necessary
if (!compareRaster(map1, map2)) {
  map2 <- resample(map2, map1, method = "bilinear")  # Adjust map 2 to map 1
}

# Jaccard Function
jaccard <- function(map1, map2) {
  map1_vals <- as.vector(getValues(map1))
  map2_vals <- as.vector(getValues(map2))
  
  # Calculate intersection/union
  intersec <- sum((map1_vals == 1) & (map2_vals == 1), na.rm = TRUE)
  union <- sum((map1_vals == 1) | (map2_vals == 1), na.rm = TRUE)
  
  return(intersec / union)
}

# Calculate Jaccard index
jaccard_index <- jaccard(map1, map2)
print(paste("Jaccard Index:", jaccard_index))

# Difference map
diff_map <- (map1 == 1) - (map2 == 1)
plot(diff_map, main = " ", col = c("blue", "white", "red"))

# ============================================================
# 9 - Apply under cloud mask
# ============================================================
...
adjust_raster <- function(raster_path, reference) {
  ...
  # Adjust the extent of the raster to match the reference raster
  cat("  Checking raster extent\n")
  if (ext(r) != ext(reference)) {
    cat("  Different extents. Cropping raster...\n")
    r <- crop(r, ext(reference))
...
# Call function to adjust the raster
ajustar_raster <- function(caminho_raster, referencia) {
  cat("Processing:", basename(caminho_raster), "\n")
  r <- rast(caminho_raster)
  
  # Check and adjust the CRS
  if (!compareCRS(r, referencia)) {
    cat("  Different CRS. Reprojecting...\n")
...
for (i in seq_along(arquivos_entrada)) {
  cat("Processing raster", i, "of", length(arquivos_entrada), "\n")
  try({
    raster_ajustado <- ajustar_raster(arquivos_entrada[i], raster_referencia)
...
# Verify if any raster was adjusted
...
Atributos <- stack # redefined stack as "Atributos"
plot(Atributos) 
print(Atributos)
summary(Atributos)

# Load the saved model
modelo_rf <- readRDS("D:\\Dissertation\\Test_Area\\Saved\\RF2\\Models\\Final_Model")

# Perform prediction using the RF model
predicao_rf <- raster::predict(Atributos, modelo_rf, progress = "text", type = "response")

# Display the results
plot(predicao_rf)
summary(predicao_rf)

predicao <- "D:\\Dissertation\\Test_Area\\Saved\\prediction_step8.tif"
writeRaster(predicao_rf, filename = predicao, overwrite = TRUE)

===============================================================
  ===============================================================
  #  10 - Combine RF2 with the previous flood map (RF1)
  # ==================================================
# **Combine with RF1 flood map**

# Load the flood map generated by RF1
mapa_rf1 <- rast("D:\\Dissertation\\Test_Area\\Saved\\RF1\\Classifications\\Class_2.tif")

# Replace the masked areas of RF1 with RF2 predictions
mapa_final <- ifel(is.na(mapa_rf1), predicao_rf, mapa_rf1)

# Visualize and export the final map
plot(mapa_final)
writeRaster(mapa_final, "D:\\Dissertation\\Test_Area\\Saved\\Final_Map\\complete_step8.tif", overwrite = TRUE)
